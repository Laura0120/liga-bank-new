...
const CheckboxCar = (props) => {
  ...
  return (
      <div >
        <div>
          <input className='calculator__checkbox' type='checkbox' id='car-insurance' checked={isCarInsurance} onChange={(evt) => setIsCarInsurance(evt.target.checked)} />
          <label htmlFor='car-insurance'>Оформить КАСКО в нашем банке</label>
        </div>
        <div>
          <input className='calculator__checkbox'type='checkbox' id='life-insurance' checked={isLifeInsurance} onChange={(evt) => setIsLifeInsurance(evt.target.checked)} />  
