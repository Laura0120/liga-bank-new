...
const Offer = () => {
  const {parameters: { type,  minSize}, amountCredit, currentInterestRate, monthlyPayment, setIsOfferRequestFormOpen, isValidPrice} = useCreditCalculatorContext();
  ...
  return  (
        <div className='calculator__offer offer'>
          ...
          : <React.Fragment><h3>Наше предложение</h3>
              <dl className='offer__content'>
                ...
                <dt>{Math.round(monthlyPayment/100*MAX_RATE_OF_INCOME) + CURRENCIES[0]}</dt>
                  <dd>Необходимый доход</dd>
              </dl>
              <button className='offer__button button' type='button' onClick={()=> setIsOfferRequestFormOpen(true) } disabled={!isValidPrice}>Оформить заявку</button>
            </React.Fragment>
        }
        </div>)
